...
from survey.views import ConfirmView, IndexView, SurveyCompleted, SurveyDetail, QueueAssignment
from survey.views.survey_result import serve_result_csv

urlpatterns = [

    url(r"^$", IndexView.as_view(), name="survey-list"),
    url('index/', IndexView.as_view(), name="survey-list"),
    ...
    url(r"^(?P<id>\d+)-(?P<step>\d+)-(?P<assignment_id>\d+)/", SurveyDetail.as_view(), name="survey-detail"),
